...
class PVTPage(QWidget):
	...
	def __init__(self) -> None:
		...
		self.load_button = QPushButton("Load Example PVT")
		self.clear_button = QPushButton("Clear PVT")
		...
		tbl_hdr_box.addWidget(QLabel("PVT Data"))
		card_layout.addWidget(tbl_hdr)
		self.table_preview = QTableWidget()
		self.table_preview.setColumnCount(3)
		self.table_preview.setHorizontalHeaderLabels(["Table", "Pressure (psia)", "Value"])
	...
	def set_project(self, project_config: ProjectConfig) -> None:
		n = len(project_config.pvt_tables)
		rows = sum(len(pts) for pts in project_config.pvt_tables.values())
		self.status_label.setText(f"{n} tables  |  {rows} data points" if n > 0 else "No tables loaded")
		self.table_preview.setRowCount(rows)
		row_idx = 0
		for tbl_name, points in project_config.pvt_tables.items():
			for pressure, value in points:
				self.table_preview.setItem(row_idx, 0, QTableWidgetItem(tbl_name))
				self.table_preview.setItem(row_idx, 1, QTableWidgetItem(f"{pressure:.2f}"))
